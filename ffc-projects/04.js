...
US phone number based on any combination of the formats provided above. The area code is required. If the country code is provided, you must 
confirm that the country code is 1. Return true if the string is a valid US phone number; otherwise return false.
*/

function telephoneCheck(str) {
  const validPatterns = [
    /^\d{3}\d{3}\d{4}$/, //matches 5555555555
    /^1?[(]\d{3}[)]\d{3}[-]\d{4}$/, //matches 1(555)555-5555 and (555)555-5555
    /^1 \d{3} \d{3} \d{4}$/, //matches 1 555 555 5555 and 555 555 5555
    /^1? ?\(\d{3}\) \d{3}-\d{4}/, //matches 1 (555) 555-5555 and (555) 555-5555
    /^1? ?\d{3}-\d{3}-\d{4}$/, //matches 1 555-555-5555 and 555-555-5555
  ];
